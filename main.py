...
def verify_password(plain, hashed): return bcrypt.checkpw(
    plain.encode('utf-8'), hashed.encode('utf-8'))


def get_password_hash(password): return bcrypt.hashpw(
...
@app.get("/")
async def read_root(): return FileResponse(
    Path(__file__).parent / "templates" / "index.html")

# --- NEW: DOWNLOAD FILE TO DEVICE ENDPOINT ---


@app.get("/download-file/{filename}")
async def download_file(filename: str):
    # Look for the file in our temp_downloads folder
...
async def register(user: UserCreate, db: Session = Depends(get_db)):
    ...
    db.add(User(username=user.username, hashed_password=get_password_hash(
        user.password), theme_color=user.theme_color))
    db.commit()
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    return {"access_token": create_access_token({"sub": user.username}), "token_type": "bearer", "username": user.username, "is_premium": user.is_premium, "theme_color": user.theme_color}


@app.get("/me")
async def get_me(current_user: User = Depends(get_current_user)):
    return {"username": current_user.username, "is_premium": current_user.is_premium, "theme_color": current_user.theme_color}
...
async def add_favorite(fav: FavoriteCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    existing = db.query(Favorite).filter(
        Favorite.user_id == current_user.id, Favorite.url == fav.url).first()
    if existing:
        return {"status": "already_exists"}
    db.add(Favorite(user_id=current_user.id, title=fav.title,
           url=fav.url, format_type=fav.format_type))
    db.commit()
    return {"status": "success"}
...
async def get_favorites(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    favs = db.query(Favorite).filter(Favorite.user_id ==
                                     current_user.id).order_by(Favorite.added_at.desc()).all()
    return [{"id": f.id, "title": f.title, "url": f.url, "format_type": f.format_type, "added_at": f.added_at.strftime("%Y-%m-%d %H:%M")} for f in favs]
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if not desc or len(desc) < 50:
                summary = [
                    f"Video: {title}", "No detailed description provided.", "Watch the video to learn more!"]
            else:
                lines = [line.strip() for line in desc.split('\n') if line.strip() and len(
                    line.strip()) > 30 and not line.startswith('http')]
                summary = lines[:3] if lines else [
                    f"About: {title}", "Check the video for full details."]
...
async def get_stats(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    total_downloads = db.query(DownloadHistory).filter(
        DownloadHistory.user_id == current_user.id, DownloadHistory.status == 'completed').count()
    ...
    return {"total_downloads": total_downloads, "total_favorites": total_favorites, "days_member": days_member}
...
@app.post("/download")
async def download_endpoint(request: DownloadRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if not current_user.is_premium:
        today = datetime.utcnow().date()
        count = db.query(DownloadHistory).filter(DownloadHistory.user_id == current_user.id,
                                                 DownloadHistory.downloaded_at >= datetime(today.year, today.month, today.day)).count()
        if count >= 5:
            raise HTTPException(
    ...
    if is_playlist and not current_user.is_premium:
        raise HTTPException(
            status_code=403, detail="Playlist downloading is a Premium feature! Upgrade to Pro.")

    download_id = str(uuid.uuid4())
    download_progress[download_id] = json.dumps({"status": "starting..."})

    db_download = DownloadHistory(user_id=current_user.id, url=request.url,
                                  format_type=request.format_type, status="in_progress", downloaded_at=datetime.utcnow())
    db.add(db_download)
    db.commit()
    ...
    def run_download():
        result = download_media(request.url, request.format_type, is_premium=current_user.is_premium,
...
@app.get("/api/history")
async def get_history(format_type: str = "all", db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    query = db.query(DownloadHistory).filter(
        DownloadHistory.status == 'completed', DownloadHistory.user_id == current_user.id)
    if format_type != "all":
        query = query.filter(DownloadHistory.format_type == format_type)
    return [{"id": h.id, "title": h.title, "url": h.url, "format_type": h.format_type, "filename": h.filename, "date": h.downloaded_at.strftime("%Y-%m-%d %H:%M")} for h in query.order_by(DownloadHistory.downloaded_at.desc()).all()]
